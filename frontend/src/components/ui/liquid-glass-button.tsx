...
// Inline cn utility — removes @/lib/utils path alias dependency entirely
...
export interface ButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  asChild?: boolean
  variant?: 'default' | 'destructive' | 'cool' | 'outline' | 'secondary' | 'ghost' | 'link'
...
const buttonVariants = ({ variant = 'default', size = 'default', className = '' }: { variant?: keyof typeof buttonVariantClasses.variant; size?: keyof typeof buttonVariantClasses.size; className?: string } = {}) => {
  const base = "inline-flex items-center cursor-pointer justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0"
  const vClass = buttonVariantClasses.variant[variant] || buttonVariantClasses.variant.default
  const sClass = buttonVariantClasses.size[size] || buttonVariantClasses.size.default
...
const liquidBaseClass = "inline-flex items-center transition-colors justify-center cursor-pointer gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-[color,box-shadow] disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none shrink-0 [&_svg]:shrink-0 outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px]"

const liquidVariantMap: Record<LiquidVariant, string> = {
...
const liquidSizeMap: Record<LiquidSize, string> = {
  default: "h-9 px-4 py-2",
...
const liquidbuttonVariants = ({ variant = 'default' as LiquidVariant, size = 'xxl' as LiquidSize, className = '' } = {}) =>
  cn(liquidBaseClass, liquidVariantMap[variant], liquidSizeMap[size], className)
