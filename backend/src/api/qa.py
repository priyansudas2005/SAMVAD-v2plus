...
def ask_meeting_question(
    meeting_id: str, payload: QAHistoryCreate, db: Session = Depends(get_db)
):
    ...
    try:
        ...
        qa_result = qa_system.answer_question(
            meeting_id, payload.question, full_transcript
        )
...
def save_qa_feedback(
    meeting_id: str,
    qa_id: int,
    payload: QAFeedbackSchema,
    db: Session = Depends(get_db),
):
    db_qa = (
        db.query(DBQAHistory)
        .filter(DBQAHistory.id == qa_id, DBQAHistory.meeting_id == meeting_id)
        .first()
    )
    if not db_qa:
