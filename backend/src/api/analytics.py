    ...
    try:
        ...
        for m in sorted_meetings:
            ...
            short_date = (
                m_date.split("T")[0]
                if "T" in m_date
                else (m_date.split(" ")[0] if m_date else "Unknown")
            )
            timeline.append(
                TimelineStat(
                    date=short_date,
        ...
        model_distribution = [
            ModelStat(name=name, value=count) for name, count in model_counts.items()
        ]
        ...
        raise HTTPException(
            ...
            detail="Failed to calculate analytics summary",
        )
