...
class ActionExtractor:
    ...
    def __init__(self) -> None:
        ...
        # Load deadline rules
        self.relative_terms = mi_cfg.get("deadline_rules", {}).get(
            "relative", ["tomorrow", "next Monday", "end of month"]
        )
    ...
    def extract(self, segments: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
                    ...
                    actions.append(
                        {
                            ...
                            "owners": owners,
                            "owner": (
                                ", ".join(owners) if owners else "UNKNOWN"
        ...
        logger.info(
            f"Extracted and merged down to {len(merged_actions)} enriched action items."
        )
    ...
    def _resolve_owners(self, text: str, default_speaker: str) -> List[str]:
        ...
        phrase_matches = re.findall(
            r"\b([A-Z][a-z]+)\s+(?:will|to|should)\s+(?:deploy|fix|write|implement|handle|test|review|setup)",
            text,
        )
    ...
    def _classify_status(self, text: str) -> str:
        ...
        if any(
            w in text_lower for w in ["blocked", "stuck", "waiting on", "depends on"]
        ):
            return "BLOCKED"
        if any(w in text_lower for w in ["completed", "done", "finished", "resolved"]):
            return "COMPLETED"
        if any(
            w in text_lower for w in ["working on", "in progress", "started", "doing"]
        ):
            return "IN_PROGRESS"
        return "TODO"
    ...
    def _calculate_confidence(
        self, text: str, owners: List[str], deadline: str
    ) -> float:
        score = 0.5  # Baseline
        ...
        if any(
            w in text.lower()
            for w in ["need to", "must", "should", "action item", "todo"]
        ):
            score += 0.2
        ...
        for act in actions:
            ...
            for existing in merged:
                ...
                ratio = SequenceMatcher(
                    None, act["task"].lower(), existing["task"].lower()
                ).ratio()
