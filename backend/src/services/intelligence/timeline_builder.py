...
class TimelineBuilder:
    ...
    def build_timeline(
        self, segments: List[Dict[str, Any]], topics: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        num_phases = min(
            max(1, int(duration / 300)), 6
        )  # 5-min intervals, max 6 phases
        ...
        for i in range(num_phases):
            ...
            phase_speakers = list(
                set(s.get("speaker_label", "UNKNOWN") for s in phase_segments)
            )

            # Dominant Speaker
            speakers_list = [s.get("speaker_label", "UNKNOWN") for s in phase_segments]
            dominant_spk = (
                Counter(speakers_list).most_common(1)[0][0]
                if speakers_list
                else "UNKNOWN"
            )
            ...
            dominant_topic = (
                max(set(phase_keywords), key=phase_keywords.count)
                if phase_keywords
                else "General"
            )
    ...
    def _classify_phase_name(
        self, segments: List[Dict[str, Any]], progress_pct: float
    ) -> str:
        ...
        if progress_pct >= 80:
            if any(
                w in all_text
                for w in ["wrap up", "thank you", "bye", "see you", "closing"]
            ):
                return "Closing"
        ...
        if any(
            w in all_text
            for w in ["code", "architecture", "database", "api", "tech", "implement"]
        ):
            return "Technical Review"
        if any(
            w in all_text for w in ["idea", "think", "suggest", "brainstorm", "concept"]
        ):
