...
class ModelLoader:
    ...
    def load_model(cls, config: STTConfig) -> WhisperModel:
        ...
        with cls._lock:
            ...
            try:
                logger.info(
                    f"Attempting to load model '{config.model_size}' on {device} ({compute_type})"
                )
                model = cls._instantiate_model(
                    config.model_size, device, compute_type, config.models_dir
                )
                cls._cached_models[cache_key] = model
                return model
            except Exception as e:
                logger.warning(
                    f"Failed loading model on {device} with {compute_type}: {e}. Retrying fallback..."
                )
        ...
        if device == "cuda":
            try:
                ...
                model = cls._instantiate_model(
                    config.model_size, "cpu", "int8", config.models_dir
                )
        ...
        if config.model_size != "tiny":
            try:
                ...
                raise ModelLoadError(
                    f"All model fallback loads failed. Tiny model load error: {tiny_err}"
                )
    ...
    def _instantiate_model(
        cls, size: str, device: str, compute_type: str, download_root: Path
    ) -> WhisperModel:
