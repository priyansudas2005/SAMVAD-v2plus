...
class HtmlExporter(BaseExporter):
    ...
    def export(
        ...
        section_order = template_cfg.get(
            "section_order", ["summary", "intelligence", "analytics", "transcript"]
        )
...
""")
        ...
        html.append(
            f"<div><h1>{meeting_title}</h1><p style='margin:4px 0 0;font-size:13px;opacity:.7'>{date_str} | {company}</p></div>"
        )
        ...
        html.append(
            "<button class='btn' style='background:#b91c1c;color:#fff' onclick='resetFilters()'>Reset</button>"
        )
        html.append("</div></div>")

        # Export metadata
        html.append(
            f"<div class='export-meta'>Export UUID: {meta['export_uuid']} | Checksum: {meta['checksum']} | Generated by SAMVAD V2.0 — Offline Verified</div>"
        )
        ...
        if "summary" in section_order and memo:
            ...
            html.append(
                "<h2 onclick='toggleCollapse(this)' class='collapse-btn'>Executive Summary</h2>"
            )
            html.append(
                f"<div class='collapse-target'><p>{memo.get('summary', 'No summary generated.')}</p>"
            )
        ...
        if "transcript" in section_order and cfg.get("include_transcript", True):
            ...
            html.append(
                "<h2 onclick='toggleCollapse(this)' class='collapse-btn'>Transcript</h2>"
            )
            html.append("<div class='collapse-target'>")
            html.append("<div style='display:flex;gap:8px;margin-bottom:8px'>")
            html.append(
                "<input type='text' id='searchBar' placeholder='Search transcript...' oninput='filterTranscript()' style='flex:2'>"
            )
            html.append(
                "<select id='speakerFilter' onchange='filterTranscript()' style='flex:1'><option value=''>All Speakers</option>"
            )
            seen = set()
            for s in segments:
            ...
            html.append("</select>")
            html.append(
                "<select id='confFilter' onchange='filterTranscript()' style='flex:1'><option value='0'>Any Confidence</option><option value='0.8'>≥ 80%</option><option value='0.5'>≥ 50%</option><option value='0.3'>≥ 30%</option></select>"
            )
            html.append("</div>")
            html.append(
                "<div id='segmentCount' style='font-size:12px;opacity:.7;margin-bottom:8px'></div>"
            )
            ...
            for seg in segments:
                ...
                html.append(
                    f"<div class='segment' style='border-left-color:{color}' data-speaker='{speaker}' data-confidence='{cx:.2f}'>"
                )
                html.append(
                    f"<span class='ts' style='color:{color}'>[{seg.get('start', '00:00')} - {seg.get('end', '00:00')}]</span> "
                )
                html.append(
                    f"<span class='speaker-tag' style='color:{color}'>{speaker}</span>"
                )
                html.append(
                    f"<span style='float:right;font-size:11px;opacity:.6'>{cx:.0%}</span>"
                )
                html.append(
                    f"<div style='margin-top:4px'>{seg.get('text', '')}</div></div>"
                )
        ...
        if "intelligence" in section_order and intelligence:
            ...
            html.append(
                "<h2 onclick='toggleCollapse(this)' class='collapse-btn'>Meeting Intelligence</h2>"
            )
            html.append("<div class='collapse-target'>")
            actions = intelligence.get("action_items", [])
            if actions:
                html.append(
                    "<h3>Action Items</h3><table><tr><th>Task</th><th>Assignee</th><th>Priority</th><th>Deadline</th><th>Status</th></tr>"
                )
                for item in actions:
                    ...
                    b = (
                        "badge-h"
                        if p == "HIGH"
                        else ("badge-m" if p == "MEDIUM" else "badge-l")
                    )
                    html.append(
                        f"<tr><td>{item.get('task','')}</td><td>{item.get('owner','')}</td>"
            ...
            if entities and cfg.get("include_entities", True):
                html.append(
                    "<h3>Entities</h3><table><tr><th>Name</th><th>Type</th><th>Freq</th><th>Confidence</th></tr>"
                )
                for ent in entities:
                    html.append(
                        f"<tr><td>{ent.get('name','')}</td><td>{ent.get('type','')}</td>"
            ...
            if topics and cfg.get("include_topics", True):
                ...
                for t in topics:
                    ...
                    html.append(
                        f"<span class='badge badge-vh' style='margin:2px'>{name}</span> "
                    )
                html.append("</p>")
            html.append("</div></div>")
        ...
        if "analytics" in section_order:
            ...
            if analytics:
                ...
                html.append(
                    "<h2 onclick='toggleCollapse(this)' class='collapse-btn'>Analytics</h2>"
                )
                html.append("<div class='collapse-target'>")
                ...
                for label, key in metrics:
                    ...
                    html.append(
                        f"<div class='stat-box'><div class='stat-val'>{val}</div><div class='stat-lbl'>{label}</div></div>"
                    )
                html.append("</div>")
                other = [
                ...
                for label, key in other:
                    ...
                    if key == "total_speaking_time_sec":
                        val = round(val / 60, 1)
                    html.append(
                        f"<p style='font-size:13px;margin:4px 0'><strong>{label}:</strong> {val}</p>"
                    )
                html.append("</div></div>")
        ...
        if kg:
            html.append(
                "<div class='card'><h2 onclick='toggleCollapse(this)' class='collapse-btn'>Knowledge Graph</h2>"
            )
            html.append("<div class='collapse-target'>")
            nodes = kg.get("nodes", [])
            edges = kg.get("edges", [])
        ...
        if timeline and cfg.get("include_timeline", True):
            html.append(
                "<div class='card'><h2 onclick='toggleCollapse(this)' class='collapse-btn'>Timeline</h2>"
            )
            html.append("<div class='collapse-target'>")
            for evt in timeline[:15]:
                ...
                html.append(
                    f"<p style='font-size:12px;margin:4px 0'><strong>{ts}</strong>: {desc}</p>"
                )
            html.append("</div></div>")
