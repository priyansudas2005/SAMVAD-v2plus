        ...
        if intelligence:
            ...
            if actions and cfg.get("include_action_items", True):
                sheets["Action Items"] = pd.DataFrame(
                    {"Task": a.get("task", ""), "Assignee": a.get("owner", ""),
                     "Priority": a.get("priority", ""), "Deadline": a.get("deadline", ""),
                     "Status": a.get("status", "")} for a in actions
                )
            ...
            if topics and cfg.get("include_topics", True):
                sheets["Topics"] = pd.DataFrame(
                    {"Topic": t.get("name") if isinstance(t, dict) else str(t)} for t in topics
                )
