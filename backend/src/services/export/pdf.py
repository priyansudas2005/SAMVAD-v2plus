...
def _build_html(meeting_title: str, date_str: str, segments: list,
    ...
    html.append("body { font-family: Helvetica, Arial, sans-serif; line-height: 1.7; color: #1e293b; max-width: 800px; margin: 0 auto; padding: 20px 40px; }")
    html.append(f"h1 {{ color: {primary_color}; border-bottom: 3px solid {accent}; padding-bottom: 10px; }}")
    html.append(f"h2 {{ color: #334155; }}")
    html.append("table { width: 100%; border-collapse: collapse; margin: 16px 0; font-size: 13px; }")
    html.append("th, td { border: 1px solid #cbd5e1; padding: 8px 10px; text-align: left; }")
    html.append("th { background: #f1f5f9; }")
    html.append("</style></head><body>")

    html.append(f"<h1>{meeting_title}</h1>")
    html.append(f"<p><strong>Date:</strong> {date_str} | <strong>Export:</strong> {meta['export_uuid']}</p>")
    ...
    if intelligence:
        ...
        if actions:
            html.append("<h2>Action Items</h2><table><tr><th>Task</th><th>Owner</th><th>Priority</th></tr>")
            for a in actions:
                html.append(f"<tr><td>{a.get('task', '')}</td><td>{a.get('owner', '')}</td><td>{a.get('priority', '')}</td></tr>")
            html.append("</table>")
    ...
    for seg in segments:
        ...
        html.append(f"<p style='border-left:4px solid {color};padding-left:12px;margin:8px 0;'>")
        html.append(f"<strong style='color:{color};'>[{seg.get('start', '00:00')}] {speaker}:</strong> ")
...
def _build_pdf_fpdf2(meeting_title: str, date_str: str, segments: list,
                     memo: Dict[str, Any] = None,
...
class PdfExporter(BaseExporter):
    ...
    def export(self, meeting_title: str, date_str: str, segments: list,
        ...
        try:
            pdf_bytes = _build_pdf_fpdf2(meeting_title, date_str, segments, memo, intelligence)
            if pdf_bytes:
                return pdf_bytes
        except Exception:
            pass
