...
class TranscriptDatabase:
    ...
    def save_transcript(
        self, meeting_id: str, timestamped_segments: List[Dict[str, Any]]
    ):
        ...
        try:
            with self._get_connection() as conn:
                ...
                conn.commit()
            logger.info(
                f"Saved {len(timestamped_segments)} transcript segments for meeting {meeting_id}"
            )
        except Exception as e:
        ...
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute(
                    ...
                    VALUES (?, ?, ?, ?)
                """,
                    (meeting_id, question, answer, datetime.now().isoformat()),
                )
                conn.commit()
