...
class SemanticRetriever:
    ...
    def index_transcript(
        self, meeting_id: Any, chunks: List[Dict[str, Any]] = None, db=None
    ) -> None:
        ...
        db.query(DBTranscriptEmbedding).filter(
            DBTranscriptEmbedding.meeting_id == meeting_id
        ).delete()
    ...
    def retrieve(
        self, meeting_id: str, question: str = None, top_k: int = None
    ) -> List[Dict[str, Any]]:
        ...
        try:
            records = (
                db.query(DBTranscriptEmbedding)
                .filter(DBTranscriptEmbedding.meeting_id == meeting_id)
                .all()
            )
            if not records:
                db.close()
            ...
            for i, score in enumerate(scores):
                ...
                results.append(
                    {"text": chunk_texts[i], "score": float(score), "chunk_index": i}
                )
