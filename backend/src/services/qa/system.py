...
class QuestionAnswering:
        ...
        try:
            ...
            if memo:
                return {
                    ...
                    "action_items": (
                        json.loads(memo.action_items_json)
                        if memo.action_items_json
                        else []
                    ),
                    "decisions": (
                        json.loads(memo.decisions_json) if memo.decisions_json else []
                    ),
                    "key_points": (
                        json.loads(memo.key_points_json) if memo.key_points_json else []
                    ),
                }
    ...
    def _query_ollama(self, question: str, context: str) -> Optional[str]:
        ...
        try:
            ...
            logger.warning(
                f"Ollama local RAG query failed, falling back to extractive: {e}"
            )
        return None

    def answer_question(
        self, meeting_id: str, question: str, transcript: str
    ) -> Dict[str, Any]:
        ...
        if is_speakers_query and not any(
            p in q_clean for p in ["say", "what did", "ask", "question"]
        ):
            speaker_turns = re.findall(r"(Speaker\s+[A-Z\d]+)", transcript)
            unique_speakers = sorted(list(set(speaker_turns)))
            intro_names = []
            matches = re.findall(
                r"\b(?:my name is|this is|colleague[s]?|introduced|call me|i am|for)\s+([A-Z][a-z]+)",
                transcript,
            )
            for m in matches:
                if m not in intro_names and m.lower() not in [
                    "everyone",
                    "someone",
                    "today",
                    "the",
        ...
        consolidated = self.context_builder.build_context(
            candidates, self.answerer.tokenizer
        )
        ...
        calibrated_score = self.confidence_calibrator.calibrate_confidence(
            best["confidence"], best.get("retrieval_score", 0.5)
        )
        ...
        return {
            "answer": final_ans,
            "confidence": calibrated_score,
            "confidence_label": self.confidence_calibrator.get_confidence_label(
                calibrated_score
            ),
            "source_snippet": best["source_chunk"][:300],
